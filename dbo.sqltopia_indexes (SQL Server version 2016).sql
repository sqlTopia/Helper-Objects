...
RETURN  WITH cteIndexes
        AS (
                ...
                OUTER APPLY     (
                                        SELECT          CONCAT(', DATA_COMPRESSION = ', dc.type_desc, ' ON PARTITIONS (', STUFF(p.info.value(N'(.)[1]', N'NVARCHAR(MAX)'), 1, 2, N''), N')')
                                        FROM            (
                                                                VALUES  (0, N'NONE'),
                                                                        (1, N'ROW'),
                                                                        (2, N'PAGE'),
                ...
                WHERE           ind.type >= 1
                                AND     (
                                                @column_name IS NULL
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), k.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), i.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), p.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), o.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), c.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= CAST(1 AS INT)
