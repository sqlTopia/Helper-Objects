        ...
        AS (
                SELECT          sch.name COLLATE DATABASE_DEFAULT AS schema_name,
                                ...
                                CASE
                                        ...
                                        ELSE N''
                                END AS bounding_box,
                                CONCAT(N'GRIDS = (LEVEL_1 = ', sit.level_1_grid_desc, N', LEVEL_2 = ', sit.level_2_grid_desc, N', LEVEL_3 = ', sit.level_3_grid_desc, N', LEVEL_4 = ', sit.level_4_grid_desc, N')') AS grids,
                                CONCAT(N', CELLS_PER_OBJECT = ', sit.cells_per_object) AS cells_per_object,
                                ind.is_disabled,
                                CASE
                                        WHEN ind.optimize_for_sequential_key = 1 THEN N'OPTIMIZE_FOR_SEQUENTIAL_KEY = ON'
                                        ELSE N'OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF'
                                END AS optimize_for_sequential_key,
                                co.content AS column_store_order
                ...
                OUTER APPLY     (
                                        SELECT          STRING_AGG(QUOTENAME(col.name) + CASE WHEN ic.is_descending_key = 1 THEN N' DESC' ELSE N' ASC' END, N', ') WITHIN GROUP (ORDER BY ic.key_ordinal)
                ...
                OUTER APPLY     (
                                        SELECT          STRING_AGG(QUOTENAME(col.name), N', ') WITHIN GROUP (ORDER BY ic.index_column_id)
                                        FROM            sys.index_columns AS ic
                                        INNER JOIN      (
                                                                SELECT  object_id,
                                                                        column_id,
                ...
                OUTER APPLY     (
                                        SELECT          STRING_AGG(QUOTENAME(col.name), N', ') WITHIN GROUP (ORDER BY ic.partition_ordinal)
                                        FROM            sys.index_columns AS ic
                                        INNER JOIN      (
                                                                SELECT  object_id,
                                                                        column_id,
                ...
                OUTER APPLY     (
                                        SELECT          STRING_AGG(QUOTENAME(col.name), N', ') WITHIN GROUP (ORDER BY ic.column_store_order_ordinal)
                                        FROM            sys.index_columns AS ic
                                        INNER JOIN      (
                                                                SELECT  object_id,
                                                                        column_id,
                                        ...
                                        WHERE           ic.object_id = ind.object_id
                                                        ...
                                                        AND ic.is_included_column = 1
                                                        AND ic.partition_ordinal= 0
                                                        AND ic.column_store_order_ordinal >= 1
                                        GROUP BY        ic.index_id
                                ) AS co(content)
                OUTER APPLY     (
                                        SELECT          COUNT(*) AS items,
                                                        STRING_AGG(l.content, N', ')
                                        FROM            (
                                                                SELECT          par.index_id,
                                                                                CONCAT(N'DATA_COMPRESSION = ', par.data_compression_desc, CASE WHEN dsp.type = 'PS' THEN CONCAT(N' ON PARTITIONS (', STRING_AGG(par.partition_number, N', ') WITHIN GROUP (ORDER BY par.partition_number)) ELSE N'' END, N')')
                ...
                WHERE           ind.type >= 1
                                AND     (
                                                @column_name IS NULL
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), k.content) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), i.content) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), p.content) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), o.content) >= CAST(1 AS INT)
                                                OR @column_name IS NOT NULL AND CHARINDEX(QUOTENAME(@column_name), c.content) >= CAST(1 AS INT)
        ...
        CROSS APPLY     (
                                SELECT  N'crix' AS action_code,
                                        CASE
                                                ...
                                                WHEN cte.index_type_major = CAST(5 AS TINYINT) AND cte.is_memory_optimized = CAST(1 AS BIT) THEN CONCAT(N'ALTER TABLE ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N' ADD INDEX ', QUOTENAME(cte.index_name), N' CLUSTERED COLUMNSTORE', CASE WHEN cte.column_store_order > N'' THEN CONCAT(N' ORDER(', cte.column_store_order, ')') ELSE N'' END , N' WITH (', cte.compression_delay, N')', cte.data_space_definition, N';')
                                                WHEN cte.index_type_major = CAST(5 AS TINYINT) THEN CONCAT(N'CREATE CLUSTERED COLUMNSTORE INDEX ', QUOTENAME(cte.index_name), N' ON ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), CASE WHEN cte.column_store_order > N'' THEN CONCAT(N' ORDER(', cte.column_store_order, ')') ELSE N'' END, N' WITH (', cte.drop_existing, N', ', cte.compression_delay, N', ', cte.data_compression, N')', cte.data_space_definition, N';')
                                                -- Spatial index
