...
/*
        sqltopia_datatype_rules v1.7.5 (2020-12-03)
        (C) 2009-2020, Peter Larsson
*/
RETURNS TABLE
AS
RETURN  WITH cteRules(schema_id, schema_name, table_id, table_name, column_id, column_name, rule_name, definition, precheck)
        AS (
                SELECT          sch.schema_id,
                                ...
                                CONCAT(N'EXISTS (SELECT * FROM sys.columns AS cc INNER JOIN sys.tables AS tbl ON tbl.object_id = cc.object_id AND tbl.name COLLATE DATABASE_DEFAULT = N', QUOTENAME(tbl.name COLLATE DATABASE_DEFAULT, N''''), N' INNER JOIN sys.schemas AS sch ON sch.schema_id = tbl.schema_id AND sch.name COLLATE DATABASE_DEFAULT = N', QUOTENAME(sch.name COLLATE DATABASE_DEFAULT, N''''), N' WHERE cc.name COLLATE DATABASE_DEFAULT = N', QUOTENAME(col.name COLLATE DATABASE_DEFAULT, N''''), N' AND cc.rule_object_id <> 0)') AS precheck
                FROM            sys.columns AS col
                INNER JOIN      sys.tables AS tbl ON tbl.object_id = col.object_id
                INNER JOIN      sys.schemas AS sch ON sch.schema_id = tbl.schema_id
                INNER JOIN      sys.objects AS obj ON obj.object_id = col.rule_object_id
                                        AND obj.type COLLATE DATABASE_DEFAULT = N'R'
                INNER JOIN      sys.sql_modules AS sqm ON sqm.object_id = col.rule_object_id
