...
RETURN  WITH cteForeignKeys
        AS (
                SELECT          fk.object_id AS foreign_key_id,
                                ...
                                STUFF(pcols.content.value(N'(.)[1]', N'NVARCHAR(MAX)'), 1, 2, N'') AS parent_columns,
                                ...
                                STUFF(ccols.content.value(N'(.)[1]', N'NVARCHAR(MAX)'), 1, 2, N'') AS child_columns,
                                CASE
                ...
                CROSS APPLY     (
                                        SELECT          CONCAT(N', ', CASE WHEN pc.name COLLATE DATABASE_DEFAULT = @column_name AND @new_column_name > N'' THEN QUOTENAME(@new_column_name) ELSE QUOTENAME(pc.name COLLATE DATABASE_DEFAULT) END)
                ...
                WHERE           (
                                ...
                                AND
                                (
                                        CHARINDEX(QUOTENAME(COALESCE(@new_column_name, @column_name)), pcols.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= 1
                                        OR CHARINDEX(QUOTENAME(COALESCE(@new_column_name, @column_name)), ccols.content.value(N'(.)[1]', N'NVARCHAR(MAX)')) >= 1
