...
RETURN  WITH cteDefaults(schema_id, schema_name, table_id, table_name, column_id, column_name, default_id, default_name, default_definition)
        ...
        CROSS APPLY     (
                                VALUES  (
                                                N'undf',
                                                CONCAT(N'EXEC sys.sp_unbinddefault @objname = N''', REPLACE(QUOTENAME(cte.schema_name) + N'.' + QUOTENAME(cte.table_name) + N'.' + QUOTENAME(cte.column_name), N'''', N''''''), N''';')
                                        ),
                                        (
                                                N'bidf',
                                        ...
                                        (
                                                N'drdf',
                                                CONCAT(N'DROP DEFAULT IF EXISTS ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.default_name), N';')
                                        ),
                                        (
                                                N'crdf',
