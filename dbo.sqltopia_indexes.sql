...
/*
        sqltopia_indexes v2.0.0 (2021-01-01)
        (C) 2009-2021, Peter Larsson
*/
RETURNS TABLE
AS
RETURN  WITH cteIndexes(schema_id, schema_name, table_id, table_name, column_id, column_name, index_id, index_name, is_unique, is_primary_key, is_unique_constraint, type_desc, filter_definition, with_clause, on_clause, key_columns, include_columns, partition_columns, precheck)
        AS (
                SELECT          x.schema_id,
                                x.schema_name,
                                x.table_id,
                                ...
                                CONCAT(N'EXISTS (SELECT * FROM sys.indexes WHERE name COLLATE DATABASE_DEFAULT = N', QUOTENAME(ind.index_name, N''''), N')') AS precheck
                FROM            (
                ...
                INNER JOIN      (
                                        ...
                                        FROM    sys.indexes
                                        WHERE   index_id >= 1
                                ) AS ind ON ind.table_id = ic.table_id
                ...
                INNER JOIN      (
                                ...
                                ) AS x ON x.table_id = ic.table_id
                                        AND x.column_id = ic.column_id
                INNER JOIN      (
                ...
                OUTER APPLY     (
                                        ...
                                        FROM            (
                                                                ...
                                                                WHERE   par.object_id = ic.table_id
