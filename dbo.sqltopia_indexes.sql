        ...
        CROSS APPLY     (
                                SELECT  N'crix' AS action_code,
                                        CASE
                                                ...
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) AND cte.is_unique = CAST(1 AS BIT) THEN CONCAT(N'CREATE UNIQUE NONCLUSTERED INDEX ', QUOTENAME(cte.index_name), N' ON ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N'(', cte.key_columns, N')', cte.include_columns, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) THEN CONCAT(N'CREATE NONCLUSTERED INDEX ', QUOTENAME(cte.index_name), N' ON ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N'(', key_columns, N')', cte.include_columns, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                -- Clustered index
