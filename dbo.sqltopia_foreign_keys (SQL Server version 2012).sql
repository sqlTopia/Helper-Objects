...
RETURN  WITH cteForeignKeys
        AS (
                SELECT          fk.object_id AS foreign_key_id,
                                ...
                                STUFF(CAST(pcols.content AS NVARCHAR(MAX)), 1, 2, N'') AS parent_columns,
                                ...
                                STUFF(CAST(ccols.content AS NVARCHAR(MAX)), 1, 2, N'') AS child_columns,
                                CASE
                ...
                WHERE           (
                                ...
                                AND
                                (
                                        CHARINDEX(QUOTENAME(COALESCE(@new_column_name, @column_name)), CAST(pcols.content AS NVARCHAR(MAX))) >= 1
                                        OR CHARINDEX(QUOTENAME(COALESCE(@new_column_name, @column_name)), CAST(ccols.content AS NVARCHAR(MAX))) >= 1
