        ...
        CROSS APPLY     (
                                SELECT  N'crix' AS action_code,
                                        CASE
                                                ...
                                                -- Nonclustered index
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) AND cte.is_primary_key = CAST(1 AS BIT) THEN CONCAT(N'ALTER TABLE ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N' ADD CONSTRAINT ', QUOTENAME(cte.index_name), N' PRIMARY KEY NONCLUSTERED (', cte.key_columns, N')', CASE WHEN cte.include_columns > N'' THEN N' INCLUDE ' + cte.include_columns ELSE N'' END, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) AND cte.is_unique_constraint = CAST(1 AS BIT) THEN CONCAT(N'ALTER TABLE ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N' ADD CONSTRAINT ', QUOTENAME(cte.index_name), N' UNIQUE NONCLUSTERED (', cte.key_columns, N')', CASE WHEN cte.include_columns > N'' THEN N' INCLUDE ' + cte.include_columns ELSE N'' END, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) AND cte.is_unique = CAST(1 AS BIT) THEN CONCAT(N'CREATE UNIQUE NONCLUSTERED INDEX ', QUOTENAME(cte.index_name), N' ON ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N' (', cte.key_columns, N')', CASE WHEN cte.include_columns > N'' THEN N' INCLUDE ' + cte.include_columns ELSE N'' END, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                WHEN cte.index_type_major = CAST(2 AS TINYINT) THEN CONCAT(N'CREATE NONCLUSTERED INDEX ', QUOTENAME(cte.index_name), N' ON ', QUOTENAME(cte.schema_name), N'.', QUOTENAME(cte.table_name), N' (', key_columns, N')', CASE WHEN cte.include_columns > N'' THEN N' INCLUDE ' + cte.include_columns ELSE N'' END, cte.filter_definition, N' WITH(', cte.online, N', ', cte.pad_index, N', ', cte.statistics_norecompute, N', ', cte.sort_in_tempdb, N', ', cte.ignore_dup_key, N', ', cte.allow_row_locks, N', ', cte.allow_page_locks, N', ', cte.fill_factor, N')', cte.data_space_definition, ';')
                                                -- Clustered index
